...
from gym_apollo.envs.apollo.CyberBridge import CyberBridge
from gym_apollo.envs.apollo.Dreamview import Dreamview
...
class ApolloContainer:
    ...
    def is_running(self) -> bool:
        ...
        try:
            ...
            # status == 'created'?
            # print("ssss:"+ docker.from_env().containers.get(self.container_name).status)
    ...
    def ip(self) -> str:
        ...
        try:
            assert self.is_running(), f'Instance {self.container_name} is not running.'
    ...
    def __dreamview_operation(self, op: str):
        ...
        if op == 'stop':
            ...
            print(f'Dreamview running at http://{self.ip}:{self.port}')
            self.logger.debug(
