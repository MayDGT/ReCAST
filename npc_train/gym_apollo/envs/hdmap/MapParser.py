...
from gym_apollo.envs.hdmap import load_hd_map
from gym_apollo.envs.modules.map.proto.map_pb2 import Map
from gym_apollo.envs.modules.map.proto.map_junction_pb2 import Junction
from gym_apollo.envs.modules.map.proto.map_lane_pb2 import Lane
from gym_apollo.envs.modules.map.proto.map_crosswalk_pb2 import Crosswalk
from gym_apollo.envs.modules.map.proto.map_signal_pb2 import Signal
from gym_apollo.envs.modules.map.proto.map_stop_sign_pb2 import StopSign
...
class MapParser:
    ...
    def get_lane_and_heading(self, pos, curr_lane, target_lane):
        ...
        if curr_lane == target_lane:
            ...
            # 计算点到目标线上的投影 并加30m
            pro_s = target_lst.project(pos)+10
            # 插值得到这个点的坐标 Point类型
    ...
    def find_lane(self, pos, lanes, left_lane = None):
    ...
    # def get_lane_central_curve(self, lane_id):
    #     # 得到从当前lane出发，两条lane的中心线连接在一起的中心线
    #     # 只会在直行的时候考虑左转，因此只考虑直行的车道
