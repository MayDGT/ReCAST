...
class OffPolicyAlgorithm(BaseAlgorithm):
    ...
    def _setup_learn(
        ...
        # 进行上面的处理后再调用父类的setup learn方法
        # 父类BaseAlgorithm方法对callback进行了初始化
    ...
    def learn(
        ...
        while self.num_timesteps < total_timesteps:
            # 在强化学习中，"rollout"（滚动或轨迹回放）通常指的是代理（智能体）在环境中执行学到的策略，生成一系列连续的状态、动作和奖励序列。Rollout 是评估代理性能的一种方式，也是用于数据收集和经验回放的关键概念。
            # 返回了一次episode的平均reward，已经收集到的step总数，已经收集到的episode总数，是否继续training
    ...
    def collect_rollouts(
        ...
        收集经验并将其存储到'ReplayBuffer'中--参考经验回放的概念
        :param env: The training environment 用gym写的环境
        :param callback: Callback that will be called at each step
            (and at the beginning and end of the rollout)
            在每一步会调用的callback，以及在rollout的beginning和end
        ...
        # 从training mode切换为evaluating mode
        ...
        while should_collect_more_steps(train_freq, num_collected_steps, num_collected_episodes):
            ...
            #ctr = 0
           # 一个step就进一次while，当跳出while的时候就是agent的done为true，当前就算一个episode
            while not done:
                ...
                print(f"num_steps:{self.num_timesteps}, obs:{new_obs}, reward:{reward}")
                print(infos)
            ...
            # 跳出上面的while循环有两种情况：done==true/should_collect_more_steps==false
            # 如果是一个episode结束了done==true
            if done:
