...
class CollisionOracle(OracleInterface):
    ...
    def on_new_message(self, topic: str, message, t):
        ...
        # timestep的单位是ms，2就代表2ms，产生于scenario runner，每0.1s加1
        time_step = self.last_perception.header.sequence_num
        ...
        for obs in self.last_perception.perception_obstacle:
            ...
            self.distances.append((adc_polygon.distance(obs_polygon), obs.id, round(time_step, 1)))
    ...
    def get_result(self) -> List[Tuple]:
        ...
        for dis in self.distances:
            if dis[0] == 0.0:
                violation = ('collision', dis[1], dis[2])
