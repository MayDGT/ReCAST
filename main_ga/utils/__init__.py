...
def get_logger_file(name):
    ...
    # 输出到文件
    file_handler = logging.FileHandler('test_apollorunner.log', 'w')
...
def random_numeric_id(length=5) -> List[int]:
    ...
    # return [int(str(i)*6) for i in range(1, length+1)]
...
def save_record_files_and_chromosome(generation_name: str, scenario_name: str, ch: dict, style:dict):
    """
    ...
    # 将场景中npc的robustness存储
    dest_file = os.path.join(dest, "style.json")
    with open(dest_file, 'w') as fp:
        json.dump(style, fp, indent=4)
