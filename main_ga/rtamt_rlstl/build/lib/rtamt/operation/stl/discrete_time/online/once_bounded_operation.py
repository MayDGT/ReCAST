...
class OnceBoundedOperation(AbstractOperation):
    ...
    def update(self, sample, typeop):
        ...
        #     elif gvars.rob_type==5:
            ...
            # print(out)
            exp_po = np.exp(beta*out)

            exp_po_sum = np.sum(exp_po)
            exp_po_sum_log = np.log(exp_po_sum)
            exp_ne = np.exp(-1*beta*out)
            exp_ne_sum = np.sum(exp_ne)
            exp_ne_sum_log = np.log(exp_ne_sum)
            diff = (exp_po_sum_log + exp_ne_sum_log)/beta
        ...
        # self.buffer中存放的是用and连接的所有式子的robustness
