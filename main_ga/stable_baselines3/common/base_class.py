...
class BaseAlgorithm(ABC):
    ...
    def __init__(
        ...
        # verbose记录运行过程中的信息的详细程度，0不输出任何信息，1输出设备和装饰器使用信息，2输出调试信息
        verbose: int = 0,
    ...
    def _init_callback(
        ...
        """
        :param callback: Callback(s) called at every step with state of the algorithm. 每个step调用的函数
    ...
    def _setup_learn(
        ...
        :param total_timesteps: The total number of samples (env steps) to train on要进行训练的总步数
        :param eval_env: Environment to use for evaluation.环境
        :param callback: Callback(s) called at every step with state of the algorithm.算法进行过程中每个步骤调用的回调函数
        :param eval_freq: How many steps between evaluations 在两个evaluation间间隔多少step
        :param n_eval_episodes: How many episodes to play per evaluation 每次评估有多少个episode
        :param log_path: Path to a folder where the evaluations will be saved 保存评估的文件夹的路径
