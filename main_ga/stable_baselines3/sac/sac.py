...
class SAC(OffPolicyAlgorithm):
    ...
    :param policy: The policy model to use (MlpPolicy, CnnPolicy, ...)用来近似policy函数的network
    :param env: The environment to learn from (if registered in Gym, can be str)强化学习的环境，如果来自于gym就可以是str
    :param learning_rate: learning rate for adam optimizer,adam优化器的学习率，相同的学习率将应用于所有的network
        the same learning rate will be used for all networks (Q-Values, Actor and Value function)
        it can be a function of the current progress remaining (from 1 to 0)
    :param buffer_size: size of the replay buffer replay缓冲区的大小？
    :param learning_starts: how many steps of the model to collect transitions for before learning starts在学习开始之前，要收集模型的转换步骤有多少？
    :param batch_size: Minibatch size for each gradient update每个梯度更新的最小批处理大小
    :param tau: the soft update coefficient ("Polyak update", between 0 and 1)？？什么参数
    :param gamma: the discount factor折扣系数
    :param train_freq: Update the model every ``train_freq`` steps. Alternatively pass a tuple of frequency and unit每train_freq步更新一次模型
    ...
    def __init__(
        ...
        env: Union[GymEnv, str], # Union表示一个变量可以是GymEnv或者是str的任意一种
    ...
    def learn(
        ...
        # super的参数可以省略，super(SAC, self)就是SAC的父类OffPolicyAlgorithm实例
